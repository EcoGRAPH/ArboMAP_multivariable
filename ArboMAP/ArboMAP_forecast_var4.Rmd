...
params:
  forecast_date:
    label: "Date to run the forecast for, e.g. 2023-05-20"
    value: "2023-05-20"
  state_name:
    label: Name of state
    value: South Dakota
  ...
  predictor_var2:
    ...
    value: rmean
  predictor_var3:
    label: Name of the third environmental predictor variable
    value: pr
  predictor_var4:
    label: Name of the fourth environmental predictor variable
    value: vs
  mosquito_model:
  ...
  file_human:
    ...
    value: !r file.path("data_human", "sd_human.csv")
    input: file
  file_mosquito:
    label: Mosquito pool data file
    value: !r file.path("data_mosquito", "sd_mosquito.csv")
    input: file
  file_strata:
    label: Stratification file, leave as "" if no strata
    value: !r file.path("data_strata", "SD 3strata.csv")
  file_county_sf:
    label: Spatial data, either file location or command. Use "create" to download
      the census spatial data once and create a file in data_spatial folder. Use "always_download"
      to download census spatial data each time.
    value: !r file.path("data_spatial", "sd_counties.RDS")
  file_models:
    label: Modelling formulas file
    value: !r file.path("data_models", "models_var4.txt")
    input: file
  folder_weather:
    label: Folder where weather/environmental data files are located
    value: data_weather
  year_human_start:
    label: Start year of human data to use and this will also be the start year of
      modelling
    value: 2004
  year_human_end:
    label: End year of human data to use. This will normally be the year before the
  ...
  year_mosquito_start:
    ...
    value: 2004
  year_mosquito_end:
    label: End year of mosquito data to use. This is normally the same year as the
      current/forecast year, or the last year of available mosquito information.
    value: 2022
  year_weather_start:
    label: Start year of weather/environmental data to use. This should be at least
      the year before the start of human data.
    value: 2000
  year_weather_end:
    label: End year of weather/environmental data to use. This is normally the same
      year as the current forecast year.
    value: 2022
  year_compare_vis1:
...
if (!is.null(params$dev_settings$data_weather)){
  ...
  data_env <- data_env_raw %>% 
    ...
    dplyr::select(arbo_ID, date_obs, year, year_epi, week_epi, date_epi, doy,
                  file_time, 
                  params$predictor_var1, params$predictor_var2, params$predictor_var3, params$predictor_var4) %>% 
  ...
  dplyr::summarize(
    ...
    !!rlang::sym(paste0(params$predictor_var2,
                        "_mean")) := mean(!!rlang::sym(params$predictor_var2),
                                          na.rm = TRUE),
    !!rlang::sym(paste0(params$predictor_var3,
                        "_mean")) := mean(!!rlang::sym(params$predictor_var3),
                                          na.rm = TRUE),
     !!rlang::sym(paste0(params$predictor_var4,
                        "_mean")) := mean(!!rlang::sym(params$predictor_var4),
                                          na.rm = TRUE),
...
for (v in c(params$predictor_var1, params$predictor_var2, params$predictor_var3, params$predictor_var4)) {
...
var3_anom_name <- paste0(params$predictor_var3, "_anom")
var4_anom_name <- paste0(params$predictor_var4, "_anom")
  ...
  summarize(
    ...
    !!rlang::sym(paste0(var2_anom_name, 
                        "_mean")) := mean(!!rlang::sym(var2_anom_name), 
                                          na.rm = TRUE),
    !!rlang::sym(paste0(var3_anom_name, 
                        "_mean")) := mean(!!rlang::sym(var3_anom_name), 
                                          na.rm = TRUE),
    !!rlang::sym(paste0(var4_anom_name, 
                        "_mean")) := mean(!!rlang::sym(var4_anom_name), 
                                          na.rm = TRUE),
  ...
  dplyr::left_join(data_env %>% 
                     ...
                     dplyr::select(arbo_ID,
                                   ...
                                   tidyselect::contains(c(params$predictor_var1,
                                                          ...
                                                          params$predictor_var3,
                                                          params$predictor_var4))),
...
data_env_mod <- data_env_mod %>% 
  ...
  dplyr::mutate(
    ...
    var2 = dplyr::if_else(
     ...
     var3 = dplyr::if_else(
      #predictor_var3 from the params
      is.na(!!rlang::sym(params$predictor_var3)), 
      #if NA, use doy mean value
      !!rlang::sym(paste0(params$predictor_var3, "_mean")),
      #else use what was there
      !!rlang::sym(params$predictor_var3)),
      var4 = dplyr::if_else(
      #predictor_var4 from the params
      is.na(!!rlang::sym(params$predictor_var4)), 
      #if NA, use doy mean value
      !!rlang::sym(paste0(params$predictor_var4, "_mean")),
      #else use what was there
      !!rlang::sym(params$predictor_var4)),
    ...
    var3_anom = dplyr::if_else(
      #anomaly of predictor_var1
      is.na(!!rlang::sym(paste0(params$predictor_var3, "_anom"))), 
      #if anom is NA, use 0 (no anomaly)
      0,
      #else use what was there
      !!rlang::sym(paste0(params$predictor_var3, "_anom"))),
    var4_anom = dplyr::if_else(
      #anomaly of predictor_var4
      is.na(!!rlang::sym(paste0(params$predictor_var4, "_anom"))),
      #if anom is NA, use 0 (no anomaly)
      0,
      #else use what was there
      !!rlang::sym(paste0(params$predictor_var4, "_anom"))))
...
data_env_lag <- data_env_lag %>% 
  tidyr::pivot_wider(id_cols = c(arbo_ID, date_obs),
                     names_from = lag,
                     values_from = c(var1, var2, var3, var4, var1_anom, var2_anom, var3_anom, var4_anom))
...
data_combined$var3 <- as.matrix(data_combined[, grep(x=colnames(data_combined),
                                                     pattern="^var3_[0123456789]")])
data_combined$var4 <- as.matrix(data_combined[, grep(x=colnames(data_combined),
                                                     pattern="^var4_[0123456789]")])
...
data_combined$var3_anom <- as.matrix(data_combined[, grep(x=colnames(data_combined),
                                                          pattern="^var3_anom_[0123456789]")])
data_combined$var4_anom <- as.matrix(data_combined[, grep(x=colnames(data_combined),
                                                          pattern="^var4_anom_[0123456789]")])
...
data_combined[,grep(x = colnames(data_combined), 
                    pattern="^var3_[0123456789]")] <- NULL
data_combined[,grep(x = colnames(data_combined), 
                    pattern="^var4_[0123456789]")] <- NULL
data_combined[,grep(x = colnames(data_combined), 
                    pattern="^var1_anom_[0123456789]")] <- NULL
data_combined[,grep(x = colnames(data_combined), 
                    pattern="^var2_anom_[0123456789]")] <- NULL
data_combined[,grep(x = colnames(data_combined), 
                    pattern="^var3_anom_[0123456789]")] <- NULL
data_combined[,grep(x = colnames(data_combined), 
                    pattern="^var4_anom_[0123456789]")] <- NULL
...
\fancyhead[C]{}
\fancyfoot[R]{\thepage}
\fancyfoot[L]{Compiled on `r format(Sys.time(), '%B %d, %Y')`}
\fancyfoot[C]{}
  ...
  dplyr::summarise(            
    ...
    !!rlang::sym(paste0(params$predictor_var2,
                        "_med")) := quantile(!!rlang::sym(params$predictor_var2),
                                             probs = 0.5, na.rm = TRUE),
    !!rlang::sym(paste0(params$predictor_var3,
                        "_med")) := quantile(!!rlang::sym(params$predictor_var3),
                                             probs = 0.5, na.rm = TRUE),
     !!rlang::sym(paste0(params$predictor_var4,
                        "_med")) := quantile(!!rlang::sym(params$predictor_var4),
                                             probs = 0.5, na.rm = TRUE),
    #mins
    !!rlang::sym(paste0(params$predictor_var1, 
    ...
    !!rlang::sym(paste0(params$predictor_var2,
                        "_min")) := min(!!rlang::sym(params$predictor_var2),
                                        na.rm = TRUE),
    !!rlang::sym(paste0(params$predictor_var3,
                        "_min")) := min(!!rlang::sym(params$predictor_var3),
                                        na.rm = TRUE),
     !!rlang::sym(paste0(params$predictor_var4,
                        "_min")) := min(!!rlang::sym(params$predictor_var4),
                                        na.rm = TRUE),
    #maxs
    !!rlang::sym(paste0(params$predictor_var1, 
    ...
    !!rlang::sym(paste0(params$predictor_var2, 
                        "_max")) := max(!!rlang::sym(params$predictor_var2),
                                        na.rm = TRUE),
    !!rlang::sym(paste0(params$predictor_var3, 
                        "_max")) := max(!!rlang::sym(params$predictor_var3),
                                        na.rm = TRUE),
    !!rlang::sym(paste0(params$predictor_var4,
                        "_max")) := max(!!rlang::sym(params$predictor_var4),
                                        na.rm = TRUE))
  ...
  dplyr::summarise(            
    ...
    !!rlang::sym(paste0(params$predictor_var2, 
                        "_med_yr")) := quantile(!!rlang::sym(params$predictor_var2),
                                                probs = 0.5, na.rm = TRUE),
    !!rlang::sym(paste0(params$predictor_var3, 
                        "_med_yr")) := quantile(!!rlang::sym(params$predictor_var3),
                                                probs = 0.5, na.rm = TRUE),
    !!rlang::sym(paste0(params$predictor_var4,
                        "_med_yr")) := quantile(!!rlang::sym(params$predictor_var4),
                                                probs = 0.5, na.rm = TRUE))
...
var3_med_name <- paste0(params$predictor_var3, "_med")
var4_med_name <- paste0(params$predictor_var4, "_med")
...
if (nrow(data_env_fc_yr) > 0){
  ...
  data_env_colors <- data_env_fc_yr %>% 
    ...
    dplyr::mutate(var_x = doy, 
                  ...
                  var3_y = !!rlang::sym(paste0(params$predictor_var3, "_med_yr")),
                  var3_yend = dplyr::lead(var3_y, n = 1),
                  var4_y =  !!rlang::sym(paste0(params$predictor_var4, "_med_yr")),
                  var4_yend = dplyr::lead(var4_y, n = 1)) %>% 
    ...
    dplyr::mutate(
     ...
     color_var2 = case_when(
        ...
        TRUE ~ "forgotsomething"),
      #var3
      color_var3 = case_when(
        var3_y >= !!rlang::sym(var3_med_name) & 
          var3_yend >= !!rlang::sym(var3_med_name) ~ "Higher",
        var3_y >= !!rlang::sym(var3_med_name) & 
          var3_yend <= !!rlang::sym(var3_med_name) ~ "Overlaps",
        var3_y <= !!rlang::sym(var3_med_name) & 
          var3_yend >= !!rlang::sym(var3_med_name) ~ "Overlaps",
        var3_y <= !!rlang::sym(var3_med_name) & 
          var3_yend <= !!rlang::sym(var3_med_name) ~ "Lower",
        TRUE ~ "forgotsomething"),
  color_var4 = case_when(
        var4_y >= !!rlang::sym(var4_med_name) & 
          var4_yend >= !!rlang::sym(var4_med_name) ~ "Higher",
        var4_y >= !!rlang::sym(var4_med_name) & 
          var4_yend <= !!rlang::sym(var4_med_name) ~ "Overlaps",
        var4_y <= !!rlang::sym(var4_med_name) & 
          var4_yend >= !!rlang::sym(var4_med_name) ~ "Overlaps",
        var4_y <= !!rlang::sym(var4_med_name) & 
          var4_yend <= !!rlang::sym(var4_med_name) ~ "Lower",
        TRUE ~ "forgotsomething")) %>% 
  ...
  p_env_var3_color <- ggplot() + 
    # min/max ribbon using wide data
    geom_ribbon(data = data_env_doy, 
                aes(x = doy, 
                    ymin = !!rlang::sym(paste0(params$predictor_var3, "_min")), 
                    ymax = !!rlang::sym(paste0(params$predictor_var3, "_max")),
                    fill = "minmax")) +
    #historical median series
    geom_line(data = data_env_colors,
              aes(x = doy, 
                  y = !!rlang::sym(var3_med_name),
                  color = "Historical median")) +
    #line segment for year median with different colors
    geom_segment(data = data_env_colors,
                 aes(x = var_x, xend = var_xend,
                     y = var3_y, yend = var3_yend,
                     color = color_var3),
                 size = 0.9) +
    #vertical line at the doy of requested date #linetype inside of aes to trigger legend
    geom_vline(aes(xintercept = doy_dt_epiwk_req, linetype = "forecast_doy"), color = "grey50") +
    ...
    scale_color_manual("Forecast year median\ncompared to historical median", 
                       values = env_comp_colors) +
    xlab("Day of the year") + 
    ylab(params$predictor_var3) +
    ggtitle(paste("Environmental data for", params$predictor_var3, 
                  "in", max(data_env$year_epi, na.rm=TRUE))) +
    theme_arbo_chart +
    theme(legend.position="bottom", 
          #legend gets cut off so multiple rows across various aesthetics
  ...
  p_env_var4_color <- ggplot() + 
    # min/max ribbon using wide data
    geom_ribbon(data = data_env_doy, 
                aes(x = doy, 
                    ymin = !!rlang::sym(paste0(params$predictor_var4, "_min")), 
                    ymax = !!rlang::sym(paste0(params$predictor_var4, "_max")),
                    fill = "minmax")) +
    #historical median series
    geom_line(data = data_env_colors,
              aes(x = doy, 
                  y = !!rlang::sym(var4_med_name),
                  color = "Historical median")) +
    #line segment for year median with different colors
    geom_segment(data = data_env_colors,
                 aes(x = var_x, xend = var_xend,
                     y = var4_y, yend = var4_yend,
                     color = color_var4),
                 size = 0.9) +
    #vertical line at the doy of requested date #linetype inside of aes to trigger legend
    geom_vline(aes(xintercept = doy_dt_epiwk_req, linetype = "forecast_doy"), color = "grey50") +
    ...
    scale_color_manual("Forecast year median\ncompared to historical median", 
                       values = env_comp_colors) +
    xlab("Day of the year") + 
    ylab(params$predictor_var4) +
    ggtitle(paste("Environmental data for", params$predictor_var4, 
                  "in", max(data_env$year_epi, na.rm=TRUE))) +
    theme_arbo_chart +
    theme(legend.position="bottom", 
          #legend gets cut off so multiple rows across various aesthetics
  ...
  plot(p_env_var3_color)
  plot(p_env_var4_color)
...
if (params$create_appendix){
  #child file location
  appx_file <- file.path("rmd_sections", "ArboMAP_appendix_var4.Rmd")
