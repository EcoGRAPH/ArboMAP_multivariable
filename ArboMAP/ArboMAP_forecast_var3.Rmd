...
params:
  forecast_date:
    label: "Date to run the forecast for, e.g. 2023-10-14"
    value: "2023-10-14"
  state_name:
    label: Name of state
    value: Louisiana
  ...
  file_human:
    ...
    value: !r file.path("data_human", "LA_human.csv")
    input: file
  file_mosquito:
    label: Mosquito pool data file
    value: !r file.path("data_mosquito", "LA_mosquito_data_20230628.csv")
    input: file
  file_strata:
  ...
  file_county_sf:
    ...
    value: !r file.path("data_spatial", "LA_counties.RDS")
  file_models:
    label: Modelling formulas file
    value: !r file.path("data_models", "models_var3.txt")
    input: file
  folder_weather:
...
if (!is.null(params$dev_settings$data_weather)){
  ...
  data_env <- data_env_raw %>% 
    ...
    dplyr::select(arbo_ID, date_obs, year, year_epi, week_epi, date_epi, doy,
                  file_time, 
                  params$predictor_var1, params$predictor_var2, params$predictor_var3) %>% 
...
for (v in c(params$predictor_var1, params$predictor_var2, params$predictor_var3)) {
...
data_env_lag <- data_env_lag %>% 
  tidyr::pivot_wider(id_cols = c(arbo_ID, date_obs),
                     names_from = lag,
                     values_from = c(var1, var2, var3, var1_anom, var2_anom, var3_anom))
